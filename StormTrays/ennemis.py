...
class Ennemi_IG(object):
    ...
    def bouge(self, tableau, fenetre, niveau, Liste_Mechants, coin, King):
        ...
        if self.count == round(64/self.vitesse):
            ...
            Taille = self.image.get_size()
            self.PosAbsolue = (self.posx*64+(self.count*self.vitesse*self.Dir_x), (self.posy*64+(self.count*self.vitesse*self.Dir_y))-((Taille[1]-64)/2))
            if self.BlitLife:
