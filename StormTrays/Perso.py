...
class Perso(object):
    ...
    def InvoqueAndBlitEnnemis(self, screen, ListMobsInMenu):
        i = random.randrange(7500)
        if i <= 7:
            if i == 0:
                Mob = EnnemiIM("../Ennemis/Dragon.json")
            elif i == 1:
                Mob = EnnemiIM("../Ennemis/Goblin.json")
            elif i == 2:
                Mob = EnnemiIM("../Ennemis/Wolf.json")
            elif i == 3:
                Mob = EnnemiIM("../Ennemis/Orc.json")
            elif i == 4:
                Mob = EnnemiIM("../Ennemis/Dwarf.json")
            elif i == 5:
                Mob = EnnemiIM("../Ennemis/Golem.json")
            elif i == 6:
                Mob = EnnemiIM("../Ennemis/Wolf.json")
            elif i == 7:
                Mob = EnnemiIM("../Ennemis/Goblin.json")
            Mob.pose_ennemi(screen)
            ListMobsInMenu.append(Mob)
            
        for Ennemi in ListMobsInMenu:
            Ennemi.vit(screen)
